...
void *ThreadSafeUpdateDsv(void *startingIndex)
{
    while(1)
    {
        //only first thread will copy values
        pthread_t thread = pthread_self();
        if(pthread_equal(thread,arrPthread[0]))
        {
        ...
        if(hasConverged)
            pthread_exit(NULL);
        //update DSV and synchronize
        ThreadSafe_UpdateDsv(startingIndex);
        pthread_barrier_wait(&threadBarrier);
    }
}
...
void ExecuteDissipation()
{
    ...
    {
        pthread_create(arrPthread+index,NULL,ThreadSafeUpdateDsv,arrPthreadStartIndex+index);
    }
...
void InitSimulation()
{
    ...
    scanf("%*d");
    //New persistent thread related work.. we do cyclic thread distribution
