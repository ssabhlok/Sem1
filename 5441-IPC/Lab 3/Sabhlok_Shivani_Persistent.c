...
//for updating DSV of boxes
void ThreadSafe_UpdateDsv()
{
    int idx;
    //automatically does block distribution
#pragma omp for
        for (idx = 0; idx < numGridBoxes; idx += 1) {
            float avgAdjacentTemp = 0;
            int overlap = 0;
            int index = 0;
            struct BOX *currBox = arrBox + idx;

            for (index = 0; index < currBox->topNeighborsCount; index += 1) {
                struct BOX *neighbor = currBox->topNeighbors[index];
                overlap = MIN(neighbor->upperLeftX + neighbor->width, currBox->upperLeftX + currBox->width) -
                          MAX(neighbor->upperLeftX, currBox->upperLeftX);
                if (overlap > 0)
            ...
            for (index = 0; index < currBox->leftNeighborsCount; index++) {
                ...
                overlap = MIN(neighbor->upperLeftY + neighbor->height, currBox->upperLeftY + currBox->height) -
                          MAX(neighbor->upperLeftY, currBox->upperLeftY);
                if (overlap > 0)
                    avgAdjacentTemp += overlap * arrDsv[neighbor->id];
            }
            ...
            arrTempDsv[currBox->id] = arrDsv[currBox->id] - affectRate * (arrDsv[currBox->id] - avgAdjacentTemp);
        }
#pragma omp barrier
}
...
#pragma omp parallel
    {
        while(!hasConverged)
        {
            /*if(!hasConverged)
            {
                ...
                ThreadSafe_UpdateDsv();
            }
             */
            ThreadSafe_UpdateDsv();

            //implicit barrier.. only the first thread reaching here will be updating this
#pragma omp single
            {
                memcpy(arrDsv,arrTempDsv, sizeof(float)*numGridBoxes);
                iterationCount++;
                if((GetMaxDsv() - GetMinDsv()) <= epsilon*GetMaxDsv())
                {
                    hasConverged = 1;
                }
...
void InitSimulation()
{
    ...
    scanf("%*d");
    //New persistent omp thread related work..
